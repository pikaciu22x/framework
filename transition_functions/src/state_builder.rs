...
// fn build_beacon_state_from_yaml<T: Config>(
//     from: String,
// ) -> Result<BeaconState<T>, yaml_rust::scanner::ScanError> {
//     let mut f = std::fs::File::open(from).expect("Conversion error");
//     let mut contents = String::new();
//     f.read_to_string(&mut contents).expect("Conversion error");
//     let docs: Vec<Yaml> = YamlLoader::load_from_str(&contents)?;

//     let d = &docs[0];

//     Ok(BeaconState::<T> {
//         genesis_time: d["genesis_time"].as_i64().expect("Conversion error") as u64,
//         slot: d["slot"].as_i64().expect("Conversion error") as u64,
//         fork: Fork {
...
//                 .try_into()
//                 .expect("slice with incorrect length"),
//             epoch: d["fork"]["epoch"].as_i64().expect("Conversion error") as u64,
//         },
//         latest_block_header: BeaconBlockHeader {
//             slot: d["latest_block_header"]["slot"]
//                 .as_i64()
//                 .expect("Conversion error") as u64,
...
//                 .expect("Decoding failed")[..],
//             ),
//             deposit_count: d["eth1_data"]["deposit_count"]
//                 .as_i64()
//                 .expect("Conversion error") as u64,
...
//                         .expect("Decoding failed")[..],
//                     ),
//                     deposit_count: x["deposit_count"].as_i64().expect("Conversion error") as u64,
//                     block_hash: H256::from_slice(
//                         &hex::decode(
//                             x["block_hash"]
//                                 .as_str()
...
//                 .to_vec(),
//         ),
//         eth1_deposit_index: d["eth1_deposit_index"].as_i64().expect("Conversion error") as u64,
...
//                         .expect("Decoding failed")[..],
//                     ),
//                     effective_balance: x["effective_balance"].as_i64().expect("Conversion error")
//                         as u64,
//                     slashed: x["slashed"].as_bool().expect("Conversion error"),
//                     activation_eligibility_epoch: x["activation_eligibility_epoch"]
//                         .as_i64()
//                         .expect("Conversion error")
//                         as u64,
//                     activation_epoch: x["activation_epoch"].as_i64().expect("Conversion error")
//                         as u64,
//                     exit_epoch: x["exit_epoch"].as_i64().expect("Conversion error") as u64,
//                     withdrawable_epoch: x["withdrawable_epoch"].as_i64().expect("Conversion error")
//                         as u64,
//                 })
//                 .collect::<Vec<_>>()
//                 .to_vec(),
...
//                 .expect("Conversion error")
//                 .iter()
//                 .map(|x| x.as_i64().expect("Conversion error") as u64)
//                 .collect::<Vec<_>>(),
//         ),
...
// #[cfg(test)]
// mod tests {
//     use super::build_beacon_state_from_yaml;
