...
use helper_functions::{
    beacon_state_accessors::{get_current_epoch, get_previous_epoch, get_block_root},
};
use types::{
    beacon_state::*,
    config::Config,
...
fn get_unslashed_attesting_indices<'a, T: Config + 'a>(_state: BeaconState<T>, _attestations: impl Iterator<Item = &'a PendingAttestation<T>>) -> impl Iterator<Item = ValidatorIndex> {
...
fn get_attesting_balance<'a, T: Config + 'a>(_state: BeaconState<T>, _attestations: impl Iterator<Item = &'a PendingAttestation<T>>) -> Gwei {
